...
export function setCurrentNode(node, currentComponentName) {
   const state = states.find(
      (state) => state.component?.name == currentComponentName
   );
...
function createOrGetState<T>(initialState: any | null = null): StateType<T> {
   ...
   const state = states.find(
      (item) =>
         item.id === stateId && item.component?.name === currentComponent?.name
   );

   if (state) {
      if (state.component) state.component.props = currentComponent?.props;
      return [state.value as T, state.set];
   } else {
      const { name: componentName } = currentComponent || { name: '' };
      const newState: State<any | null> = {
         id: stateId,
         value: initialState,
...
function setState(
   newStateOrCallback: any | Function,
   id: number,
   componentName: string
) {
   const state = states.find(
      (item) => item.id == id && item.component?.name === componentName
   );
   ...
   if (state && state.value !== newState) {
      ...
      /* invoke component*/
      let newVNode: VNodeType = state.component?.factory(
      ...
      if (state.component?.container)
         reconcile(newVNode, state.component?.container, state.component?.node);
   }
}

function handleFragment(vNode: VNodeType, state: State<any>) {
   let oldVNode = (state.component?.node as any)?._vNode;

   if (
      (vNode.type === Constants.Fragment ||
         vNode.type.name === Constants.Fragment) &&
      oldVNode
   ) {
      const oldChildren = [...oldVNode.children];
      oldChildren?.forEach((item, index) => {
         if (
            item.type.name === state.component?.name ||
            item.componentName === state.component?.name
         ) {
            oldChildren[index] = {
               ...vNode,
